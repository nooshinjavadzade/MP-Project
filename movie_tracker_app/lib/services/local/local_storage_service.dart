...
class LocalStorageService {
  ...
  static const String _keyAuthUser = 'cached_auth_user';
  ...
  Future<void> saveMediaList(String listKey, List<MediaBase> items) async {
    final jsonList = items.map((e) => e.toJson()).toList();
    await _instance.setString(listKey, jsonEncode(jsonList));
    await _setTimestamp(listKey);
    final trackedKeys = _instance.getStringList(_keyHomeSectionKeys) ?? [];
    if (!trackedKeys.contains(listKey)) {
      trackedKeys.add(listKey);
      await _instance.setStringList(_keyHomeSectionKeys, trackedKeys);
    }
  }

  List<MediaBase> getMediaList(String listKey, {bool checkTtl = true, Duration ttl = _defaultTtl}) {
    if (checkTtl && !_isCacheValid(listKey, ttl: ttl)) return [];
    final str = _instance.getString(listKey);
    if (str == null) return [];
    try {
      final List<dynamic> jsonList = jsonDecode(str);
      return jsonList
  ...
  Future<void> clearSessionData() async {
    ...
    await _instance.remove(_keyAuthUser);
    await _cacheManager.emptyCache();
  ...
  Future<void> saveAuthUser(User user) async {
    await _instance.setString(_keyAuthUser, jsonEncode(user.toJson()));
  }

  User? getAuthUser() {
    final str = _instance.getString(_keyAuthUser);
    if (str == null) return null;
    try {
