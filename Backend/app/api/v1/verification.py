...
from app.services.auth_service import create_tokens_and_save_refresh, log_audit_event, revoke_all_refresh_tokens
...
async def request_verify_email(
    ...
    User email is extracted from the JWT token.
    """
    ...
    await rate_limit_otp_request(current_user.email, client_ip, "verify_email")
...
async def confirm_verify_email(
    ...
    """
    await rate_limit_otp_verify(current_user.email, "verify_email")
...
async def request_password_reset(
    ...
    """
    client_ip = http_request.client.host if http_request.client else "unknown"
    await rate_limit_otp_request(request.email, client_ip, "reset_password")
...
async def confirm_password_reset(
    ...
    """
    await rate_limit_otp_verify(request.email, "reset_password")
